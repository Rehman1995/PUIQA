...
# -------------------------- Main ------------------------------
    ...
    image_folder = 'UID_2021'
    mos_file = os.path.join('MOS Values', 'mos_UID.xlsx')
    df = load_mos(mos_file)
    print("🔍 Extracting all features...")
    features_all, labels_all = extract_dataset_features(image_folder, df)
    
    #combo = ('color', 'texture', 'structure', 'frequency', 'hdaf', 'chromatic', 'multi_structure', 'multi_hdaf', 'multi_fpff')
    
    combo = tuple(features_all[0].keys())
    print(f"\n🧪 Evaluating specific combination: {combo}")
    selected_features = np.array([
        np.concatenate([f[g] for g in combo]) for f in features_all
    ])
    ...
    print(f"\n📊 Results for {combo} ➤ RMSE: {rmse:.3f}, SRCC: {srcc:.3f}, PLCC: {plcc:.3f}")
...
def main():
    ...
    combo = ['color', 'multi_texture', 'multi_frequency', 'edge_energy', 'local_entropy', 'multi_fpff']
