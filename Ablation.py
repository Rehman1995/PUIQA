# -*- coding: utf-8 -*-
"""
Created on Tue May  6 13:02:40 2025
...
# -*- coding: utf-8 -*-
"""
Created on Tue May  6 12:39:21 2025
...
def run_ablation_study(features_all, labels_all, base_combo, save_csv='ablation_results.csv'):
    results = []
    print("\n🧪 Running Ablation Study...")
    for feat in base_combo:
        ablated_combo = [f for f in base_combo if f != feat]
        X = np.array([np.concatenate([f[g] for g in ablated_combo]) for f in features_all])
        X_train, X_test, y_train, y_test = train_test_split(X, labels_all, test_size=0.2, random_state=42)
        ...
        results.append({'Removed': feat, 'RMSE': rmse, 'SRCC': srcc, 'PLCC': plcc})
        print(f"❌ Removed {feat} ➤ RMSE: {rmse:.3f}, SRCC: {srcc:.3f}, PLCC: {plcc:.3f}")
...
def plot_ablation_results(ablation_data, metric='SRCC'):
    labels = [item['Removed'] for item in ablation_data]
    values = [item[metric] for item in ablation_data]

    plt.figure(figsize=(12, 6))
    bars = plt.bar(labels, values, color='skyblue')
    plt.xticks(rotation=45)
    plt.ylabel(metric)
    plt.title(f'Ablation Study - Impact on {metric}')
    for bar in bars:
        yval = bar.get_height()
...
def main():
    ...
    # Define your selected feature groups
    combo = ['color', 'multi_texture', 'multi_frequency', 'edge_energy', 'local_entropy', 'multi_hdaf', 'multi_frequency', 'multi_fpff', 'illumination', 'specular', 'veiling_gradient']
    ...
    # 1000 evaluation runs
    rmse_list, srcc_list, plcc_list = [], [], []
    ...
    for i in range(1000):
        ...
        model = make_pipeline(StandardScaler(), SVR(kernel='rbf', C=10, epsilon=0.5, gamma='scale'))
        model.fit(X_train, y_train)
        preds = model.predict(X_test)
    ...
    # 🔬 Run Ablation Study
    ablation_results = run_ablation_study(features_all, labels_all, combo)

    # 📈 Plot Ablation Results
    for metric in ['SRCC', 'PLCC', 'RMSE']:
        plot_ablation_results(ablation_results, metric)
